...
// ─── Agenda Global (sin caso asociado) ──────────────────────────────────────

export async function addGlobalEventAction(formData: FormData) {
  const supabase = await createClient()
  const title = formData.get('title') as string
  const type = formData.get('type') as string
  const eventDate = formData.get('event_date') as string
...
export async function deleteGlobalEventAction(eventId: string) {
  const supabase = await createClient()
  const { error } = await supabase.from('events').update({ is_archived: true }).eq('id', eventId)
  if (error) throw new Error(error.message)
